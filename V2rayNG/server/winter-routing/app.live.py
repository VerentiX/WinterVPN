...
HEALTH_TIMEOUT = os.getenv("HEALTH_TIMEOUT", "8s")
...
HIDDIFY_MAX_PRIORITY = int(os.getenv("HIDDIFY_MAX_PRIORITY", "9999"))
KARING_MAX_PRIORITY = int(os.getenv("KARING_MAX_PRIORITY", "9999"))
CLASH_MI_MAX_PRIORITY = int(os.getenv("CLASH_MI_MAX_PRIORITY", "9999"))
...
HAPP_FORCE_DIRECT_DOMAINS = [
    ...
    for item in os.getenv(
        "HAPP_FORCE_DIRECT_DOMAINS",
        "keyword:mtalk.google.com,domain:mtalk.google.com",
    ).split(",")
    if item.strip()
]

HAPP_FORCE_DIRECT_PORTS = [
    item.strip()
    for item in os.getenv(
        "HAPP_FORCE_DIRECT_PORTS",
        "5228",
...
def build_happ_xray_rules(
    ...
    if not profile:
        ...
        if HAPP_FORCE_DIRECT_PORTS:
            forced_rules.append(
                {
                    "type": "field",
                    "port": ",".join(HAPP_FORCE_DIRECT_PORTS),
                    "network": "tcp",
                    "outboundTag": "direct",
                }
            )
...
def apply_mobile_sockopts(
    ...
    """Remove socket options that Xray cannot apply inside iOS NetworkExtension."""
    protocol = str(outbound.get("protocol", "")).lower()
    stream = outbound.get("streamSettings")
...
def candidate_to_lxbox_uri(candidate: dict[str, Any]) -> str:
    """Encode a VLESS candidate as a share URI accepted by Android LxBox."""
    proxy = candidate_to_mihomo(candidate)
    ...
    query: dict[str, str] = {
        ...
        "type": str(proxy.get("network") or "tcp"),
    }
    ...
    if proxy.get("tls"):
        ...
        if proxy.get("alpn"):
            query["alpn"] = ",".join(str(item) for item in proxy["alpn"])
    ...
    if isinstance(xhttp, dict):
        ...
        for source_key, target_key in xhttp_key_map.items():
            value = xhttp.get(source_key)
    ...
    return (
        f"vless://{quote(str(proxy['uuid']), safe='')}@{server}:{proxy['port']}"
        f"?{urlencode(query)}#{quote(str(proxy['name']), safe='')}"
    )


def build_clash_config(
    configs: list[dict[str, Any]],
    max_priority: int = CLASH_MAX_PRIORITY,
    mobile_whitelist: bool = False,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    priority_config = load_priority_config()
    candidates = [
        item
        for item in build_candidates(configs, priority_config)
        if 0 <= int(item["priority"]) <= max_priority
        and str(item["protocol"]).lower() == "vless"
    ]
    if not candidates:
        raise ValueError(
            f"No supported profiles with priority P0-P{max_priority}"
        )
    ...
    group_name = f"🇷🇺 АВТОВЫБОР P0-P{max_priority}"
    vpn_group = "🛡️ VPN"
    ...
    mobile_whitelist_rules = [
        ...
        "RULE-SET,torrent-domains,REJECT-DROP",
        "RULE-SET,torrent-clients,REJECT-DROP",
        "MATCH,PROXY",
    ]
    ...
    # On iOS, Clash Mi runs inside NetworkExtension and cannot reliably read
    # the DNS servers assigned to the physical Wi-Fi/cellular interface.
    # dhcp://system then falls back to public resolvers, which are unavailable
    # on allow-list-only mobile networks.  Let iOS resolve through the
    # operator-provided system resolver and keep domain routing via sniffing.
    if mobile_whitelist:
        config["tun"].pop("dns-hijack", None)
        config["dns"] = {"enable": False}
        config["profile"].pop("store-fake-ip", None)
        config["sniffer"]["skip-dst-address"] = private_networks
...
def merge_configs(
    ...
    # Resolve through the DNS service assigned by the active Wi-Fi/cellular
    # provider.  Public resolvers such as 8.8.8.8 are commonly unavailable on
    # allow-list-only networks, while Xray's localhost resolver follows the
    # operating system configuration without creating a proxy/DNS loop.
    base["dns"] = {
    ...
    # Force-direct (GMS mtalk/5228) must run before the auto-proxy-in bridge and
    # before ProxySites like geosite:google-play, or sniffed push traffic loops
    # through the loopback outbound "proxy" and burns CPU.
    force_direct_rules = [
        rule
        for rule in happ_rules
        if rule.get("outboundTag") == "direct"
        and (
            rule.get("port")
            or any(
                "mtalk" in str(item).lower()
                for item in (
                    rule.get("domain")
    ...
    other_happ_rules = [rule for rule in happ_rules if rule not in force_direct_rules]
    routing["rules"] = (
        force_direct_rules
        + chain_rules
        + [proxy_bridge_rule]
...
def attach_winter_mobile_routing_headers(
    ...
    if header_value:
        # Drop the bulky Happ Routing link for Winter-Mobile. Keeping both
        # Profile-Routing and Routing overflows nginx's default proxy header
        # buffer and yields 502, which makes Cloudflare fail over to the
        # backup generator (without Selectel P0 routes).
        headers.pop("Routing", None)
        headers.pop("routing", None)
        headers.pop("Routing-Enable", None)
        headers.pop("routing-enable", None)
...
def clash_priority_limit(request: Request) -> int:
    """Give app-specific clients every tier; keep regular Clash on P0-P4."""
    user_agent = request.headers.get("user-agent", "").lower()
    if "hiddifynext" in user_agent:
        return HIDDIFY_MAX_PRIORITY
    if "karing" in user_agent:
        return KARING_MAX_PRIORITY
    # Clash Mi currently identifies subscription requests as
    # "ClashMeta/<version>; mihomo/<version>". Clash Verge includes its own
    # "clash-verge" marker and must retain the desktop P0-P4 profile.
    if (
        "clash-verge" not in user_agent
        and "clashmeta/" in user_agent
        and "mihomo/" in user_agent
    ):
        return CLASH_MI_MAX_PRIORITY
    return CLASH_MAX_PRIORITY


def clash_mobile_whitelist(request: Request) -> bool:
    """Use strict allow-list routing for Clash Mi on mobile devices."""
    user_agent = request.headers.get("user-agent", "").lower()
    return (
        "clash-verge" not in user_agent
        and "clashmeta/" in user_agent
...
def auto_format_is_lxbox(request: Request) -> bool:
    """Return the Xray config-array subscription understood by sing-box-lx."""
    explicit_format = request.query_params.get("format", "").strip().lower()
    if explicit_format in {"lxbox", "sing-box-lx", "singbox-lx"}:
        return True

    user_agent = request.headers.get("user-agent", "").lower()
    return user_agent.startswith("lxbox/")


def auto_format_is_lxbox_android(request: Request) -> bool:
    explicit_format = request.query_params.get("format", "").strip().lower()
    if explicit_format in {"lxbox-android", "sing-box-lx-android"}:
        return True
    return request.headers.get("user-agent", "").lower().startswith(
        "lxbox-android/"
    )
...
async def balanced_subscription(
    ...
    if auto_format_is_clash(request):
        response = await clash_subscription(sub_id, request)
    ...
    try:
        ...
        if auto_format_is_lxbox_android(request):
            candidates = build_candidates(
                source_configs,
                load_priority_config(),
            )
            links = [
                candidate_to_lxbox_uri(candidate)
                for candidate in candidates
                if str(candidate.get("protocol") or "").lower() == "vless"
            ]
            if not links:
                raise ValueError("No VLESS profiles for Android LxBox")
            response = Response(
                content="\n".join(links) + "\n",
            ...
            response.headers["X-Auto-Format"] = "lxbox-android-vless"
            return response

        if auto_format_is_lxbox(request):
            lxbox_manifest = build_candidates(
                source_configs,
                load_priority_config(),
            )
            response = JSONResponse(
                content=source_configs,
                headers=copy_subscription_headers(
                    upstream.headers,
                    lxbox_manifest,
                ),
            )
            response.headers["X-Auto-Format"] = "lxbox-xray-array"
            return response
...
async def clash_subscription(sub_id: str, request: Request) -> Response:
    if not SUB_ID_RE.fullmatch(sub_id):
        raise HTTPException(
            status_code=400,
            detail="Invalid subscription ID",
    ...
    try:
        source_configs = normalize_config_list(upstream.json())
        max_priority = clash_priority_limit(request)
        config, candidates = build_clash_config(
            source_configs,
            max_priority=max_priority,
            mobile_whitelist=clash_mobile_whitelist(request),
        )
    except (ValueError, TypeError, json.JSONDecodeError) as exc:
        raise HTTPException(
            status_code=502,
    ...
    headers: dict[str, str] = {
        ...
        "Content-Disposition": (
            f'attachment; filename="zizmos-clash-auto-p0-p{max_priority}.yaml"'
        ),
        "X-Generated-By": "3x-ui-clash-priority-fallback-v1",
        "X-Balancer-Mode": "priority-fallback",
        "X-Balancer-Priorities": f"P0-P{max_priority}",
        "X-Balancer-Profiles": str(len(candidates)),
        "X-Health-Profile": "standard",
    }
