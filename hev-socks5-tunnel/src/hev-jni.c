...
thread_handler (void *data)
{
    ...
    /* The worker may also exit because of an initialization/runtime error.
     * Keep the JNI lifecycle state truthful so a later Android-side recovery
     * attempt is not silently ignored as "already working". */
    /* native_stop_service holds this mutex while joining us and will clear
     * the flag itself. For an unexpected exit the try-lock succeeds and makes
     * a future start possible, without introducing a join/mutex deadlock. */
